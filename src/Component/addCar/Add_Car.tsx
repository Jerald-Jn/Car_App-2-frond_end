...
function Add_Car(){
    ...
    function changeHandler(event: ChangeEvent<HTMLInputElement>) {
     ...
     const addCar=async(event: FormEvent<HTMLFormElement>)=>{
        ...
        const response=(await addCar_Api(formData)).data
        console.log(response)
    }
    return (
        <>
            <div className="relative bg-[url('./src/assets/car-4.jpg')] min-h-screen bg-cover bg-center">
                <form onSubmit={(event)=>addCar(event)} className="md:mx-10 mx-3 my-5 text-center md:my-16 absolute top-0 bg-slate-200/50 rounded-2xl">
