...
function Header() {
  ...
  return (
    ...
    <header className=' border-b-2 border-black/10' onMouseEnter={() => {setShowProducts(false),setShowService(false)}}>
      <nav className=''>
        <div className='flex justify-between mt-0.5 p-2 md:mx-5 md:p-5'>
          ...
          <svg onClick={()=>navigate('/')} className='w-3/5 mt-0.5 md:h-6 md:w-40 hover:cursor-pointer' onMouseEnter={() => { setShowProducts(false), setShowService(false) }} xmlns="http://www.w3.org/2000/svg" width="136" height="24" viewBox="0 0 136 24"><path fill="none" d="M0 0h158v48H0V0z" />
          ...
          <div className='hidden md:block w-4/5'>
            <ul className='flex flex-row gap-5 items-center uppercase text-xs font-bold   md:mt-2 md:-ml-1'>
              <li className='tracking-wider underline-offset-8 hover:underline hover:cursor-pointer '>
                <Link to={''} onMouseEnter={() => { setShowProducts(true), setShowService(false) }}>Products</Link>
              </li>
              <li className='tracking-wider underline-offset-8 hover:underline hover:cursor-pointer'>
                <Link to={''}   onMouseEnter={() => { setShowService(true), setShowProducts(false) }}>Service</Link>
              </li>
